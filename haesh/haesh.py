...
class Haesh(object):
    ...
    def _expand_cipherblock(self, last_cipherblock):
        ...
        for block_index in last_cipherblock:
            ...
            expanded_ords.extend([
                self.expansion_permutation[first_index],
                self.expansion_permutation[(first_index + 1) % self._expansion_value],
                self.expansion_permutation[(first_index + 2) % self._expansion_value],
                self.expansion_permutation[(first_index + 3) % self._expansion_value]
            ])
